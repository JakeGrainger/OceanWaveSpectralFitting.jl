...
In particular, a `JONSWAP{K}` model for a series recorded every `Δ` seconds would be appropriate for a JONSWAP{K} series sampled every `Δ` seconds and filtered with a high-pass filter at `(2K-1)π/Δ`.
